...
MySQL 首次初始化会执行 `deploy/docker/mysql/init/*.sql`，当前会创建认证、用户和服务市场需要的 `user`、`service`、`service_image`、`service_tag` 表。若本地已经存在旧的 `mysql-data` volume，Docker 不会重复执行初始化脚本；需要重建初始化数据时请先自行备份并清理对应 volume。
...
| `XUEYIFANG_MYSQL_URL` | `jdbc:mysql://127.0.0.1:3306/xueyifang?...` | Auth、User 和 Service 服务本地 MySQL 连接串。 |
| `XUEYIFANG_MYSQL_USERNAME` | `root` | Auth、User 和 Service 服务本地 MySQL 用户名。 |
| `XUEYIFANG_MYSQL_PASSWORD` | `123123123` | Auth、User 和 Service 服务本地 MySQL 密码。 |
