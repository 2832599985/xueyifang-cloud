...
MySQL 首次初始化会执行 `deploy/docker/mysql/init/001-user.sql`，创建认证和用户服务当前需要的 `user` 表。若本地已经存在旧的 `mysql-data` volume，Docker 不会重复执行初始化脚本；需要重建初始化数据时请先自行备份并清理对应 volume。
...
| `XUEYIFANG_MYSQL_URL` | `jdbc:mysql://127.0.0.1:3306/xueyifang?...` | Auth 和 User 服务本地 MySQL 连接串。 |
| `XUEYIFANG_MYSQL_USERNAME` | `root` | Auth 和 User 服务本地 MySQL 用户名。 |
| `XUEYIFANG_MYSQL_PASSWORD` | `123123123` | Auth 和 User 服务本地 MySQL 密码。 |
