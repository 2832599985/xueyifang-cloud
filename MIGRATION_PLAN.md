...
| 原项目分析 | 待处理 | 需要读取或拉取原后端、前端项目，梳理业务模块和接口。 |
...
- 获取原后端项目 `2832599985/xueyifang-backend`。
- 获取原前端项目 `2832599985/xueyifang-frontend`。
- 梳理后端包结构、数据库表、接口、权限模型和外部依赖。
- 梳理前端页面、路由、接口调用和角色入口。
- 输出模块盘点文档。
...
- 原项目数据库类型、表结构和初始化脚本位置。
- 前端是否同步纳入本仓库，还是后续独立仓库联调。
- 第一批业务迁移范围，建议从认证和用户开始。
