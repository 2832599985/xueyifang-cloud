...
| 阶段 5 服务市场与交易最短链路 | 进行中 | `xueyifang-service` 已接入服务浏览、发布者管理和互动链路；`xueyifang-trade` 已接入订单创建、支付、取消、发货、确认完成、退款、钱包、纠纷、订单定时任务和买卖家订单查询。 |
...
- 第一批迁移已采用“认证、用户、服务列表、订单最短链路”的顺序；后续优先补系统字典、消息通知和本地联通验证。
