...
- 当前状态：阶段 4 认证与用户迁移进行中，阶段 5 服务市场与交易最短链路已启动；阶段 3 基础设施已完成，已新增 JWT 公共能力、Gateway Bearer Token 校验、Servlet 用户上下文解析、Auth 登录/注册/刷新/登出、Redis Token 黑名单、User 当前用户资料和发布权限接口，`xueyifang-service` 已提供服务列表、详情、标签读取、服务发布、我的服务、编辑、上下架、逻辑删除、收藏、我的收藏、评价创建、评价列表和订单评价状态，`xueyifang-trade` 已提供订单创建、支付、取消、发货、确认完成、买卖家订单列表和详情接口。
...
| `deploy/docker/mysql/init/003-trade.sql` | SQL | 本地 MySQL 初始化 `service_order`、`service_order_log` 和 `wallet_transaction` 表，供交易服务使用。 |
...
| `xueyifang-trade` | 已创建 | 交易服务，当前包含 Spring Boot 启动类、MySQL/JDBC 接入、订单创建、支付、取消、发货、确认完成、买卖家订单列表和详情接口。 |
...
| `xueyifang-trade/src/main/java/com/xueyifang/cloud/trade/controller/OrderController.java` | 订单入口，提供创建、支付、取消、发货、确认完成、买卖家列表和详情接口。 |
...
| `xueyifang-trade/src/main/java/com/xueyifang/cloud/trade/service/TradeOrderService.java` | 订单状态机、权限校验、钱包冻结/结算和分页查询业务逻辑。 |
| `xueyifang-trade/src/main/resources/application.yml` | 交易服务端口、服务名、Nacos 和 MySQL 配置。 |

## Todo

- 补交易服务退款申请、卖家处理退款、纠纷和相关定时任务。
- 补钱包余额、钱包流水、充值和提现接口；资金规则复杂后再评估是否拆出钱包服务。
